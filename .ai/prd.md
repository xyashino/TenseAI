...
- `REQ-06`: New users will complete a mandatory onboarding process after registration.
- `REQ-07`: The onboarding form collects the user's name.
- `REQ-08`: The onboarding form allows the user to set a default difficulty level (Basic or Advanced).

### 3.3. Core Application Interface

- `REQ-09`: The application will use responsive web navigation: a sidebar on desktop and a fixed bottom bar on small screens.
- `REQ-10`: The navigation will contain four main sections: "Practice", "Theory", "History", and "Account".
...
- `REQ-20`: The application must persist the state of **all active sessions** for a user on the server. A user can have an **unlimited number of concurrent, active sessions**. If the user closes the site or browser tab, they will be able to resume **any of their active sessions** from the "Practice" screen upon their next visit. The Practice screen displays all active sessions and allows users to resume or delete them. A local cache may be used to improve recovery speed, but the server is the source of truth.
...
- Acceptance Criteria:
  ...
  - When I see the onboarding screen, I can enter my name and select a default difficulty level ("Basic" or "Advanced") in a single form.
...
- Acceptance Criteria:
  ...
  - When I return to the site and navigate to the "Practice" screen,
  - Then I see a list of all my unfinished sessions,
  - And I can choose which one to resume or delete.
