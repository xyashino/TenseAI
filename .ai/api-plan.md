...
4.  **[NEW] Question Generation Logic**:
      - *(Future):* Try to fetch 10 questions from the database pool matching `tense` and `difficulty`.
      - *(For MVP):* If questions are not available in the DB (or logic isn't implemented), call AI service to generate 10 new questions.
...
3.  Server **quickly** creates the `training_sessions` record *only* (status 'active') and returns `201 Created` with session data.
